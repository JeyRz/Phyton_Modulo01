...
def usuario(nombre):
    ...
    for _ in base_de_usuarios:
        if base_de_usuarios[indice].getNombreU() == nombre:
            val = indice
        indice += 1
    tarjetas_usuario = base_de_usuarios[val].getTarjetas()    
...
def reporte(nombre, tarjeta):
    ...
    indice = 0
    val = 99
    for tarj in usser.getTarjetas():
        if tarj.getNombre() == tarjeta:
            val = indice
        indice +=1
    lista_tarjetas = usser.getTarjetas()
